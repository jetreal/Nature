var gulp           = require('gulp'), // Подключаем Gulp
	...
	pngquant     		 = require('pngquant'), // Подключаем библиотеку для работы с png
...
gulp.task('css-libs', function() {
	return gulp.src('app/css/main.css') // Выбираем файл для минификации
		...
		.pipe(gulp.dest('app/css')),
				gulp.src('app/css/cssLibs/*.css') // Выгружаем в папку app/css
...
exports.build2 = series('sass', 'css-libs'); // компиляция sass в scc + сжатие
...
exports.see = parallel('browser-sync', 'watch'); // наблюдение 
...
exports.prodaction = gulp.series('clean', 'img', gulp.series('sass', 'css-libs'), 'scripts', 'build'); // сборка и перенос в dist

gulp.task('clear', function (callback) {
	return cache.clearAll();
})

gulp.task('default', gulp.parallel('watch', 'browser-sync', gulp.series('sass', 'css-libs'), 'scripts'));
